...
namespace RevitScript.Runtime.Engine {
    ...
    public class ScriptExecutor {
        ...
        public static int ExecuteScript(ScriptData scriptData, ScriptRuntimeConfigs scriptRuntimeCfg, ScriptExecutorConfigs scriptExecConfigs = null) {
            // make sure there are base configs
            scriptExecConfigs ??= new ScriptExecutorConfigs();

            if (mainThreadId == 0) return ScriptExecutorResultCodes.ExecutorNotInitialized;
            return Thread.CurrentThread.ManagedThreadId == mainThreadId ? ExecuteScriptNow(scriptData, scriptRuntimeCfg, scriptExecConfigs) : RequestExecuteScript(scriptData, scriptRuntimeCfg, scriptExecConfigs);
        ...
        {
            return runtime.ScriptRuntimeConfigs.ConfigMode switch
            {
                true => EnsureTargetScript(runtime.ScriptData.ConfigScriptPath),
                _ => EnsureTargetScript(runtime.ScriptData.ScriptPath)
            };
        }
